    ...
    block: [
      {
        ...
        rex: /\\\[(((?!\\\]|\\\[)[\s\S])+?)\\\]\s*\(([^)$\r\n]+)\)/gy,
        tmpl: '<span class="eqno"><eqn>$1</eqn><span>($2)</span></span>',
        tag: "\\[",
      },
      {
        name: "math_block",
...
function texmathPre(str: string, outerSpace: boolean, beg: number) {
  ...
  return (
    ...
    (outerSpace
      ...
      : prv !== C_BACKSLASH && (prv < C_0 || prv > C_9))
  ); // no backslash, and no decimal digit .. before opening '$'
}
...
export function markdownItTexMath(
  md: markdownExit,
  options?: { delimiters?: string; outerSpace?: boolean; katexOptions?: any; macros?: any },
): void {
  ...
  for (const rule of delimiters.inline) {
    ...
    md.renderer.rules[rule.name] = (tokens, idx) =>
      rule.tmpl.replace(
        /\$1/,
        texmathRender(tokens[idx].content, !!rule.displayMode, katexOptions),
      );
  }
