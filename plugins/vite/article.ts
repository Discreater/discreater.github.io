...
function insertHeader(headers: ArticleHeader[], header: ArticleHeader, level: number) {
  if (level === 1 || headers.length === 0) headers.push(header);
  else insertHeader(headers[headers.length - 1].children, header, level - 1);
}
...
function extractBodyIt(body: string) {
  ...
  const md = new MarkdownExit().use(Anchor, {
    callback: (_token, { slug, title }) => {
      ...
      if (!Number.isNaN(level)) insertHeader(headers, header, level);
    },
  });
  md.render(body);
  return headers;
...
async function parseArticleGitInfo(article: Article, git: SimpleGit): Promise<GitFileStatus> {
  ...
  if (dates.length === 0) dates.push(new Date().toISOString());
...
async function getAllArticlesInfo(config: InternalArticlePluginOptions): Promise<string> {
  ...
  const articleAttributesPromise = articlePaths.map((article) =>
    getArticleInfo(article, git, config.route),
  );
  const articleAttributes = (await Promise.allSettled(articleAttributesPromise)).flatMap(
    (value) => {
  ...
  articleAttributes.sort((a, b) => {
    ...
    if (a.attributes.createdAt === undefined && b.attributes.createdAt === undefined) {
      ...
      if (aDate === undefined && bDate === undefined) dateRes = 0;
      else if (aDate === undefined || bDate === undefined) return aDate === undefined ? -1 : 1;
      else dateRes = bDate.getTime() - aDate.getTime();
    }
...
function resolveOptions(config: ArticlePluginOptions): InternalArticlePluginOptions {
  ...
  if (typeof route === "string") route = (name: string) => `${config.route}/${name}`;
  return {
    path: config.path,
    route,
  };
...
export async function articlePlugin(config: ArticlePluginOptions): Promise<Plugin<any>> {
  ...
  return {
    ...
    load(id) {
      if (id === resolvedVirtualModuleId) return `export const articles = ${articles};`;
    },
  };
}
